...
from crypto_bot import config, risk as risk_mod
from crypto_bot.costs import CostModel
from crypto_bot.datasource import get_source
from crypto_bot.compare import REGISTRY
...
class _Provider:
    ...
    def __init__(self, st, df, new_start):
        if st["strategy"] == "ml":
            import numpy as np
            from crypto_bot.ml import (build_features, build_labels, MLP, MLStrategy,
                                      WARMUP, load_best_nn)
...
def tick(source: str = "yfinance", pairs=None, cost: CostModel | None = None,
    ...
    if cost is None:
        from crypto_bot.costs import CryptoCostModel
