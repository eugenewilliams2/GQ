...
from crypto_bot import data as data_mod
from crypto_bot import config, metrics, costs, risk
from crypto_bot.engine import backtest
from crypto_bot.compare import compare, REGISTRY
...
def _add_source_args(p):
    p.add_argument("--asset", default="crypto", choices=["crypto"],
                   help="forex (=X pairs) or crypto (BTC-USD ...)")
    p.add_argument("--source", default="coinbase",
...
def main() -> None:
    ...
    if args.cmd == "compare":
        ...
        from crypto_bot.report import generate
        path = generate(_load(args), args.out, periods_per_year=ppy)
        print(f"report written -> {path}")
    elif args.cmd == "validate":
        from crypto_bot.validate import holdout_validate
        ...
        from crypto_bot.pairs import oos_pairs
        ...
        from crypto_bot.native_app import main as run_app
        run_app()
    elif args.cmd == "funding":
        from crypto_bot.funding import run_funding
        ...
        from crypto_bot import autolearn
        if args.show:
        ...
        from crypto_bot.ml import nn_search
        ...
        from crypto_bot.ml import run_ml, holdout_ml
