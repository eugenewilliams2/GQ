...
from fx_bot import config, risk as risk_mod
from fx_bot.costs import CostModel
from fx_bot.datasource import get_source
...
class _Provider:
    ...
    def __init__(self, st, df, new_start):
        if st["strategy"] == "ml":
            import numpy as np
            from fx_bot.ml import (build_features, build_labels, MLP, MLStrategy,
                                      WARMUP, load_best_nn)
...
def tick(source: str = "yfinance", pairs=None, cost: CostModel | None = None,
    ...
    if cost is None:
        from fx_bot.costs import CryptoCostModel
